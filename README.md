...
幹線道路の歩道などで、車列が一時的に途切れてすぐに再び現れる場合に過剰な通知を抑制するための設定です。

**「Clear suppression time（車列クリア後の通知抑制）」**

- 設定値：0〜60秒（デフォルト: 10秒）
- 車列がクリアされてから設定値以内に再検知された場合は通知しない
- 例: 10秒に設定した場合、車列がクリアされてから10秒以内に再検知されたら通知しない
- 0秒に設定した場合は常に通知（抑制なし）

**使用例**
- 幹線道路の歩道走行時: 10〜15秒に設定
- 頻繁な通知が不要な場合: 20〜30秒に設定
- 常に通知したい場合: 0秒に設定
